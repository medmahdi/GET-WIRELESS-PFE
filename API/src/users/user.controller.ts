...
export class UserController {
    ...
    async getAllUserSmallerDto(@Res() res) {
        const users = await this.userService.getAllUserDto();
        return res.status(HttpStatus.OK).json(users);
    }
    ...
    @Put('update/:id')
    async updateUser(@Res() res, @Param('id') id, @Body() createUserDTO: any) {
    ...
    // Activate a user's details
    @Put('activate/:id')
    async activateStatus(@Res() res, @Param('id') id) {
        const user = await this.userService.activateStatus(id);
        if (!user) throw new NotFoundException('User does not exist!');
        return res.status(HttpStatus.OK).json({
            message: 'User has been successfully updated',
            user
    ...
    // Activate a user's details
    @Put('deactivate/:id')
    async deactivateStatus(@Res() res, @Param('id') id) {
        const user = await this.userService.deactivateStatus(id);
        if (!user) throw new NotFoundException('User does not exist!');
        return res.status(HttpStatus.OK).json({
            message: 'User has been successfully updated',
            user
